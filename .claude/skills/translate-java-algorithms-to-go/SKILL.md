...
1. Inspect the Java class folder (use Glob/Grep or `find` to list its files).
...
```bash
go test ./algorithm/lession/classXX
```

If the default Go build cache is unavailable in this environment, fall back to a writable cache, e.g. `env GOCACHE=/private/tmp/go-build-cache go test ./algorithm/lession/classXX`.
