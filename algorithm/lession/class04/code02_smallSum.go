...
// 题目：数组小和指每个数左边所有比它小的数累加起来的总和，要求返回整个数组的小和。
// 暴力做法枚举每个数左侧所有元素，时间复杂度 O(N^2)。
// 核心思路：归并时如果左侧 arr[p1] 小于右侧 arr[p2]，因为右侧区间有序，arr[p1] 会对 p2..r 的所有数产生贡献。
// 利用这个批量结算能力，把统计小和嵌入归并排序过程，整体复杂度降到 O(NlogN)。
// 时间复杂度：O(NlogN)。
// 空间复杂度：O(N)。
