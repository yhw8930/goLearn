...
// 题目：数组中有一种或两种数出现奇数次，其余数都出现偶数次，找出奇数次的数。
// 所有数整体异或时，出现偶数次的数会完全抵消。
// 如果只有一种奇数次的数，最终异或结果就是它；如果有两种，最终结果是 a^b。
// 核心思路：用 a^b 最右侧的 1 把数组分成两组，a 和 b 会被分到不同组中分别异或出来。
// 时间复杂度：O(N)。
// 空间复杂度：O(1)。
...
// arr中，有一种数，出现奇数次
// printOddTimesNum1 处理只有一种数出现奇数次的情况。
// 全体异或后，偶数次出现的数被抵消，剩下的就是答案。
...
// printOddTimesNum2 处理有两种数出现奇数次的情况。
// 先得到 a^b，再用最右侧的 1 把 a 和 b 分到不同组中分别异或。
// 时间复杂度：O(N)。
// 空间复杂度：O(1)。
