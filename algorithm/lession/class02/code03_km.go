...
// 题目：数组中只有一种数出现 K 次，其他所有数都出现 M 次，且 K < M，找出出现 K 次的数。
// 要求不用哈希表时，可以从整数二进制位的角度统计。
// 对每一位统计数组中有多少数在该位为 1，出现 M 次的数贡献一定能被 M 整除。
// 核心思路：每一位对 M 取模后剩下的就是目标数在该位的状态。
// 前提：K < M，且除一种数外其他数都出现 M 次。
// 时间复杂度：O(N)。
// 空间复杂度：O(1)，位数固定为 32。
...
// onlyKTimes 是不使用哈希表的主算法。
// 它按二进制位统计所有数的 1 出现次数，再对 M 取模还原出现 K 次的数。
// 时间复杂度：O(N)。
// 空间复杂度：O(1)，只使用固定 32 位计数数组。
