...
// 题目：给定字符串，打印它所有字符的全排列。
// 排列要求使用所有字符，区别在于字符出现顺序。
// 核心思路：一种做法是维护剩余字符列表，每次选择一个字符追加到路径。
// 另一种做法是在原字符数组上交换 index 和后续位置；去重时每一层用 set 避免相同字符重复作为当前位置。
// 时间复杂度：O(N!*N)。
// 空间复杂度：O(N) 递归深度，不计结果集；保存结果需要 O(N!*N)。
