...
// 题目：给定两个单链表头节点，链表可能有环也可能无环，返回它们第一个相交节点。
// 如果两个链表不相交，返回 nil。相交的含义是节点地址相同，不是节点值相同。
// 核心思路：先分别用快慢指针判断两个链表是否有环，并找到各自入环节点。
// 之后分三类处理：都无环按长度差找交点；一个有环一个无环必不相交；都有环再判断入环点相同或环内相遇。
// 时间复杂度：O(N+M)。
// 空间复杂度：O(1)。
