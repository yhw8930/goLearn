...
// 题目：给定由 1 和 0 组成的二维网格，1 表示陆地，0 表示水，求岛屿数量。
// 上下左右相邻的陆地属于同一座岛，斜方向不算连通。
// 核心思路：感染法遇到一块未访问陆地就把整座岛递归改掉，岛屿数加一。
// 也可以把每块陆地作为并查集节点，合并相邻陆地，最后集合数就是岛屿数。
// 时间复杂度：O(M*N)。
// 空间复杂度：感染法递归最坏 O(M*N)，并查集 O(M*N)。

// ================================================================================
// 解法一：感染法 (DFS) - 最简单
// ================================================================================
// numIslands3 是感染法版本。
// 每发现一块陆地，就递归或扩散把整座岛改成已访问状态。
// 时间复杂度：O(M*N)。
// 空间复杂度：最坏 O(M*N)，递归感染深度。
...
// numIslands1 是通用对象并查集版本。
// 它把陆地包装成节点对象，再合并相邻陆地。
// 时间复杂度：O(M*N) 近似，Union/Find 近似 O(1)。
// 空间复杂度：O(M*N)，对象和哈希表并查集。
...
// numIslands2 是数组并查集版本。
// 它把二维坐标映射成一维下标，减少对象开销。
// 时间复杂度：O(M*N) 近似。
// 空间复杂度：O(M*N)，数组并查集。
