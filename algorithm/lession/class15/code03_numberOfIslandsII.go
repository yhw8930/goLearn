...
// 题目：初始全是水的 m*n 网格，按 positions 逐步加入陆地，返回每一步后的岛屿数量。
// 同一位置可能重复加入陆地，重复操作不应该改变岛屿数量。
// 核心思路：每次新陆地先单独形成一个集合，岛屿数加一。
// 然后检查上下左右四个方向，若邻居已经是陆地就合并集合，成功合并一次岛屿数减一。
// 时间复杂度：数组并查集每次近似 O(1)，总计 O(K) 近似；K 为加陆地次数。
// 空间复杂度：O(M*N) 或哈希表 O(K)。
...
// numIslands21 使用固定数组并查集。
// 适合网格尺寸已知且可以预先分配 M*N 空间的场景。
// 时间复杂度：O(K) 近似，K 为 positions 数量。
// 空间复杂度：O(M*N)，预分配整张网格。
...
// numIslands22 使用哈希表并查集。
// 只为实际出现的陆地建记录，适合稀疏加点场景。
// 时间复杂度：O(K) 近似。
// 空间复杂度：O(K)，只保存实际加入的陆地。
