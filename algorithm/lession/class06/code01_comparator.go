...
// 题目：演示如何用比较器定义结构体排序规则。
// 比较器不是算法本身，而是告诉排序过程两个元素谁应该排在前面。
// 核心思路：比较函数返回负数、零、正数分别表示第一个元素更靠前、相等或更靠后。
// 可以先按主字段比较，主字段相等时再按次字段比较，从而组合复杂排序规则。
// 时间复杂度：排序取决于 sort 实现，通常 O(NlogN)。
// 空间复杂度：取决于排序实现和输入。
