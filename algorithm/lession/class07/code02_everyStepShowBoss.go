...
// 题目：用户发生购买或退货事件后，每一步都返回当前得奖区 TopK 用户。
// 用户购买数会动态变化，得奖区和候选区之间还要按进入时间打破平局。
// 核心思路：候选区用“大购买数优先”的加强堆，得奖区用“小购买数优先”的加强堆。
// 每次事件后先更新用户购买数和所在区域，再比较两个堆顶决定是否替换得奖区成员。
// 时间复杂度：每个事件调整堆为 O(logN)。
// 空间复杂度：O(N)。
