...
// 题目：给定项目利润 profits、启动资金 capital，最多做 K 个项目，初始资金 W，求最终最大资金。
// 只有当前资金不小于项目启动资金时，该项目才可以选择；每个项目最多做一次。
// 核心思路：小根堆按启动资金保存所有未解锁项目，把当前资金能做的项目弹出放入利润大根堆。
// 每轮从利润大根堆选择利润最高项目做掉并增加资金，重复最多 K 轮。
// 时间复杂度：O(NlogN + KlogN)。
// 空间复杂度：O(N)。
